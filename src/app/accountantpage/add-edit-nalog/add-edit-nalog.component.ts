    ...
    if (this.myForm.valid) {
        if(this.dataService.getRezervacija()!=undefined){
          this.dataService.updateRezervarion(rez).subscribe(
            data => { if (data["_body"].indexOf('updated')>=0) { this.router.navigate(['racunovodja']); } else {alert("Greska prilikom izmene rezervacije!!!") }
            }
          );
        }else{
          ...
          this.dataService.addRezervation(rez).subscribe(
            data => { if (data["_body"].indexOf('ok')>=0) { this.router.navigate(['racunovodja']); } else {alert("Greska prilikom unosa nove rezervacije!!!") }
            }
          );
      }
    }else{
