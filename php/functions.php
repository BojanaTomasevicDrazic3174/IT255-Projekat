...
function addCar($idAutomobil,$ID_TARIFNA_KLASA,$ID_OSIGURANJE,$MARKA_AUTOMOBILA,$MODEL_AUTOMOBILA,$DATUM_KUPOVINE,
...
function addClient($idClientA,$IME_KLIJENTA,$PREZIME_KLIJENTA,$ADRESA_KLIJENTA,$DATUM_RODJENJA,$TELEFON,$EMAIL,$USERNAME,$PASSWORD,$tip) {
  	global $conn;
  	$rarray = array();
...
function updateRezervarion($ID_REZERVACIJA,$ID_AUTOMOBIL,$idClientA,$ID_TRAJANJA,$DATUM_REZARVACIJE,
	...
	$rarray = array();
		...
		$stmt->bind_param("iiisssisiisi",$ID_AUTOMOBIL,$idClientA,$ID_TRAJANJA,$DATUM_REZARVACIJE,
...
function addRezervation($ID_REZERVACIJA,$ID_AUTOMOBIL,$idClientA,$ID_TRAJANJA,$DATUM_REZARVACIJE,
													$DATUM_POCETKA,$DATUM_ZAVRSETKA,$VALIDNA_REZERVACIJA,$NACIN_REZERVISANJA,$PREDJENA_KILOMETRAZA,$CENA,$STATUS) {
  	global $conn;
  	$rarray = array();
  	$stmt = $conn->prepare("INSERT INTO `rezervacija` ( `ID_AUTOMOBIL`, `ID_KLIJENTA`, `ID_TRAJANJA`, `DATUM_REZARVACIJE`,
														`DATUM_POCETKA`, `DATUM_ZAVRSETKA`, `VALIDNA_REZERVACIJA`, `NACIN_REZERVISANJA`, `PREDJENA_KILOMETRAZA`, `CENA`, `STATUS`)
                            VALUES ( ?, ?,?, ?, ?,?, ?, ?,?,?,?)");
  	$stmt->bind_param("iiisssisiis",$ID_AUTOMOBIL,$idClientA,$ID_TRAJANJA,$DATUM_REZARVACIJE,
															$DATUM_POCETKA,$DATUM_ZAVRSETKA,$VALIDNA_REZERVACIJA,$NACIN_REZERVISANJA,$PREDJENA_KILOMETRAZA,$CENA,$STATUS);
  	if($stmt->execute()){
  		$rarray['sucess'] = "ok";
  	}else{
